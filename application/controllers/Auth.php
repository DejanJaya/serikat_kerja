...
class Auth extends CI_Controller
{
    ...
    {
        $this->form_validation->set_rules('nik', 'Nik', 'numeric|required|is_unique[tb_anggota.nik]|min_length[16]|max_length[16]', ['required' => '%s harus diisi', 'is_unique' => '%s tidak boleh sama', 'numeric' => '%s harus angka', 'max_length' => '%s Harus 16 karakter', 'min_length' => '%s Harus 16 karakter']);
        $this->form_validation->set_rules('nama', 'Nama', 'required', ['required' => '%s harus diisi']);
        $this->form_validation->set_rules('username', 'Username', 'required|is_unique[tb_anggota.username]', ['required' => '%s harus diisi', 'is_unique' => '%s tidak boleh sama']);
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $tambah = $this->M_all->simpan('tb_anggota', $data);
            if ($tambah) {
                echo "<script>";
    ...
    public function masuk()
    {
        ...
        $cek_user = $this->M_all->tampilSatu('tb_anggota', $where);
